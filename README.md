...
| **数值未经真人试玩定标** | `PLAYER` / `CAMERA` / `WEAPON` / `ENEMY` / `WARDEN` / `DIRECTOR` / `ITEMS` 是起始基线。没有读数就不调参——**"一局 10–15 分钟"仍是纸面推算** |
| **Bloom 后处理未开启** | 重开条件是拿到真机开 / 关两次帧率读数；没有读数就不开 |
| **Web 端只验到 HTTP 层** | 构建、相对引用、子目录托管（Pages 项目站的形状）与字节一致性都验过，但**开发机是受限沙箱、起不了任何 Chromium 进程**，所以"页面在浏览器里真的跑起来了吗"这一条一直只能靠你点一次。上一轮现场反馈的两个启动缺陷（静默失败 / 脱绑定方法调用）都已修并补了回归测试，见 `docs/PROJECT_TECHNICAL_PLAN.md` §5.13 / §5.14 |
...
src/
  ...
  render/      表现层：相机 · 场景 · 敌人视图 · HUD · 对象池 · 特效 · 模型加载器
...
深挖请读 **`docs/PROJECT_TECHNICAL_PLAN.md`**：§0 是进度快照与证据表，§5.6–§5.15 是各阶段实施纪要（§5.15 是 Web 上线），§6.3 是全部定案决策。
