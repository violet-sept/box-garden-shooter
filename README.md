...
一个**能立刻上手、单手就能玩明白**的小型 3D 箱庭射击游戏：你站在一块被围栏圈起的立体沙盘里，先用步枪点掉分五批落下来的 30 只小敌人，再去面对那只慢慢走过来的大块头——它会用**一条黄色的直线**把你从掩体后面逼出来；把它放倒之后还有**第二批敌人**：一架**在天上绕着你飞**的武装直升机。场地上散落着**弹药箱与医疗箱**，`E` 键捡起来就是补给。

**一局的形状是固定的脚本，不再是按曲线加码的波次**：开局先给你 **10 秒空场**（屏幕顶上一条倒计时 `第一批敌人还有 N 秒到达战场`），然后 **30 只潜袭者分五批、每隔 10 秒落一批**（5 / 5 / 5 / 5 / 10，最后一批在 t = 50 s，都带 0.9 s 的落地预警）；**把最后一只也清掉、场上没有任何别的东西之后**，典狱长才出场——一条命里只有它一只，而且**没有超时兜底**：不去清掉最后一只潜袭者，就永远见不到它。**打死典狱长也不是终点**：屏幕顶上那行倒计时换成 `第二批敌人还有 N 秒到达战场`（20 秒），到点之后**武装直升机**登场——**攻击方式、血量与攻击力都和典狱长一致**，区别是它在天上飞、绕着你转圈。**把它也打掉，这一局才算赢。**
...
| **`Q`** | 投掷道具（抛物线，1.4s 引信，5.5m 溅射）。**阶段 11 起是 `Q`**——此前是 `E`，而 `E` 现在专职拾取 |
| **`E`** | **拾取补给箱**：走到**金色弹药箱**（备弹 **+90 发**）或**白底红十字医疗箱**（生命 **+50**）旁边按一下。够得着时屏幕下方会出现 `按 E 拾取 …` 的提示；用完箱子立刻消失，按住了也不会连着吃 |
| `Shift` | 疾跑 |
...
**开局那 10 秒，屏幕顶上有一条倒计时**（`第一批敌人还有 N 秒到达战场`，从 10 数到 1，警告色）：它只属于开局——后面四批不再倒数，按阶段 3 以来的老规矩用**地面光环 + 提示音**宣告落点。**同一行字在典狱长倒下之后会回来一次**（`第二批敌人还有 N 秒到达战场`，20 秒），那就是直升机降临的倒计时。
...
**第二批敌人是一架武装直升机**，它的数字**就是从典狱长那张表里读出来的**：同样是 **4800 血、34 点伤害、1.35 s 抬手、2.8 s 冷却**，连那六个弹道旋钮都是同一组——需求是"与典狱长一致"，所以代码里是同一个来源，而不是两份抄得像的数字。不一样的地方只有两处：它**飞在 11 m 高**、**绕着你以 16 m 半径转圈**（蓄力时它会停下来，和典狱长一样——那条黄线必须读得懂）。**橙色螺旋桨在黑色机身上一直在转**，那也是它的蓄力提示。⚠️ **它是从天上往下打的**：货箱这类矮掩体基本拦不住它，找高墙或者高台。

**场上有两种补给箱，按 `E` 捡**：**金色弹药箱**给备弹 **+90 发**，**白底红十字医疗箱**给 **+50 点生命**。它们**每 20 秒每种各刷一个**（刷在地图上随机、且不会跟已有的箱子或箱子里的墙重叠），**每种最多同时存在 2 个**——满了就先不刷，你在场上取走一个之后，下一轮它又会补上。用过的箱子会立刻消失。**补给箱是这一版唯一的弹药来源**：两场 4800 血的仗按弱点效率要 274 发，而一局只带 240 发，所以**不捡箱子在算术上是打不完的**。
...
| 文件 | 原始 | gzip |
|---|---|---|
| `dist/index.html` | 11.33 kB | 4.11 kB |
| `dist/assets/index-tqglj7uR.js` | 134.44 kB | 45.87 kB |
| `dist/assets/three-DQQBLnPL.js` | 630.37 kB | 158.80 kB |

three 单独分包（`manualChunks`），因为它几乎不变、而游戏代码经常变；改一次游戏代码的玩家只需要重新下载那 134 kB。
...
| **阶段 11 的新内容也一条都没被人看过** | **直升机的样子**（黑色机身 + 橙色旋翼在天上是什么观感、旋翼转得够不够明显、从头顶往下打一条黄线是压迫感还是"打不着"）· **它让掩体失效**（11 m 高打下来的线，1.2 m 的货箱基本拦不住——这是"会飞的敌人"应有的样子，但也可能让后半局变成站桩对射）· **20 秒的补给节奏跟不跟得上两场 4800 血的仗**（一局自带 240 发，两场弱点仗要 274 发，**不捡箱子算术上打不完**）· 黄金 / 白色两个箱子在场地里够不够显眼。**要调就按这个顺序**：`PICKUPS.refreshInterval` / `ammoRounds` → `WEAPON.reserveAmmo` → `HELICOPTER.altitude` / `orbitRadius`；**别动直升机那 4800 与 34**（"与典狱长一致"是需求） |
...
src/
  ...
  game/        纯逻辑：玩家 · 武器 · 敌人 AI（两个重甲共用 `heavyAttacker.ts`）· 导演与投放（`director/deployment.ts`）· 投掷物 · 补给箱（`pickups/`）· 关卡几何
  render/      表现层：相机 · 场景（含补给箱视图）· 敌人视图（含直升机旋翼）· 玩家身体（rig/转身）· HUD · 对象池 · 特效 · 模型加载器
